...
import (
	...
	"github.com/VinGitonga/gin-auth/modules/auth"
	"github.com/VinGitonga/gin-auth/modules/config"
	"github.com/VinGitonga/gin-auth/modules/database"
	"github.com/VinGitonga/gin-auth/modules/database/query"
	"github.com/VinGitonga/gin-auth/modules/encrypt"
...
func (ga *GoApp) SignUp() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		switch status {
		case 1:
			ctx.JSON(http.StatusOK, gin.H{"message": "Registration Successfully"})
			return
	...
	return func(ctx *gin.Context) {
		...
		if ok {
			...
			if err != nil {
				...
				ctx.JSON(http.StatusUnauthorized, gin.H{"message": "Incorrect login credentials"})
			...
			case verified:
				...
				if err != nil {
					_ = ctx.AbortWithError(http.StatusInternalServerError, fmt.Errorf("token not generated: %v", err))
				...
				// update the db, adding the token to user database
...
func (ga *GoApp) Dashboard() gin.HandlerFunc {
	return func(ctx *gin.Context) {
		ctx.JSON(http.StatusOK, gin.H{"resp": "Welcome to App Dashboard"})
	}
}
